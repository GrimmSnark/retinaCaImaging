function map = bandpassFermiFilterGPU(map,wavelengthLower,wavelengthUpper,spatialResolution,zscoreData,T,ROI)
...
function [paddedImGPU startingPosition] = padImage(im,paddingValue)
...
paddedImGPU = gpuArray(paddingValue.*ones(paddedImageSize));
...
paddedImGPU(startingPosition(1):(startingPosition(1)+size(im,1)-1),startingPosition(2):(startingPosition(2)+size(im,2)-1)) = im;
